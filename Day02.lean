...
def checkForInvalid (range : String × String) (size : Nat) : Nat :=
...
def solve1 (input : String) : String :=
  ...
  let totals := splitRanges'.map (λ r => checkForInvalid r (ceilDiv r.fst.length 2)) 
  toString (totals.sum)
  --let invalidIds := splitRanges.map findInvalid
